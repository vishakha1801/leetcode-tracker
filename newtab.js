...
class LeetCodeDashboard {
    ...
    setupEventListeners() {
        ...
        // Disconnect on click of data-source indicator
    ...
    isCacheValid(data) {
        return data?.timestamp && (Date.now() - data.timestamp < 1000 * 60 * 15);
    }
    ...
    updateUI() {
        ...
        const streakSubtitle = document.querySelector('#month-solved')?.parentElement
            ?.parentElement?.querySelector('.stat-subtitle');
        if (streakSubtitle) {
            streakSubtitle.textContent = `${this.data.streaks.current} day streak`;
        }

        const yearElem = document.getElementById('heatmap-year');
        if (yearElem) {
            yearElem.textContent = `Recent Contributions`;
        }
    ...
    generateContributionHeatmap() {
        try {
            ...
            // First Sunday before or on start date to align grid
            const firstSunday = new Date(start);
            firstSunday.setDate(firstSunday.getDate() - firstSunday.getDay());

            // Replace month label generation logic in generateContributionHeatmap()
            const monthWeeks = {};
            days.forEach(d => {
                const month = d.toLocaleString('default', { month: 'short' });
                const year = d.getFullYear(); // Include year in key
            ...
            Object.values(monthWeeks).forEach(monthData => {
                ...
                label.textContent = monthData.month; // Only show month name
                label.style.left = `${centerPosition}px`;
                label.style.transform = 'translateX(-50%)'; // Center alignment
            ...
            //tooltip.style.cssText = 'position:absolute;background:rgba(0,0,0,0.9);color:white;padding:6px;border-radius:6px;font-size:0.8rem;opacity:0;transition:opacity .2s;';
